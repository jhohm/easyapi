  ...
  return function(req, res, next) {
    ...
    if(Array.isArray(api_version)) {
      ...
      if(version_index != -1) {
        ...
        return send(res, { notFound: 'Unknown API Version' }, 404);
      }
     } else {
    ...
    for (; i < components.length; i += 2) {
      ...
      if (!resource) {
        return send(res, { notFound: 'Resource Not Found' }, 404);
      }
    ...
    if (!resource || ! resource[method]) {
      return send(res, { notFound: 'Method Not Found' }, 404);
    }
    
    if(resource.auth && !req.api_auth) {
      return send(res, { auth: 'Method Requires Authentication'}, 401);
    }
